...
-> /cmd_vel_mpc -> fake/chassis velocity transform -> /cmd_vel
...
导航侧的权威速度输出是 `/cmd_vel_mpc`，实机经过既有 `fake_vel_transform` 与
`chassis_vel_transform` 后以 `/cmd_vel` 发送到下位机。下位机负责 CAN、电机、舵轮、
...
- `/cmd_vel_mpc` 必须保持 `ats_swerve_mpc` 的唯一发布者；
