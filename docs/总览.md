...
这个入口用于单独观察 `plan`、`smoothed_path_visual`、`trajectory_profile_markers`、`trajectory_esdf_debug` 和速度链，不混入视觉跟随或行为树额外变量。
...
- `trajectory_optimizer.*` / `smoother_server.bspline_smoother.*`
  控制 B 样条平滑、ESDF obstacle refinement、轨迹 profile。
- `trajectory_speed_governor.*`
  控制根据近端曲率窗口进行的二次限速。
- `inflation_radius`
  控制离墙保守程度。
...
3. 改 MPPI、goal checker、costmap、ESDF 和速度 profile，优先看 Nav2 参数文件
4. 改下位机模式和速度下发，优先看 `standard_robot_pp_ros2`
5. 查速度慢，按 `/trajectory_profile -> /cmd_vel_controller -> /cmd_vel_controller_governed -> /cmd_vel_nav2_result -> /cmd_vel` 分级定位
6. loopback 与实车共用行为层核心算法，因此调试结论优先先在行为层对齐

## 9. 建议阅读顺序

1. [./omni_recovery_smoothing_optimization.md](./omni_recovery_smoothing_optimization.md)
2. [./上车测试清单.md](./上车测试清单.md)
3. [./mppi_parameter_tuning_guide.md](./mppi_parameter_tuning_guide.md)
4. [./sentry_bt_decision_checklist.md](./sentry_bt_decision_checklist.md)
5. [./sentry_posture_switch_logic.md](./sentry_posture_switch_logic.md)
6. [./slim_loopback_refactor.md](./slim_loopback_refactor.md)
7. [./融合.md](./融合.md)
8. [./视觉跟随仿真调试.md](./视觉跟随仿真调试.md)
9. [./实机视觉跟随优化方案.md](./实机视觉跟随优化方案.md)
