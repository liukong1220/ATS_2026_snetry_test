...
## 0.0 2026-07-31 Nav2-free 配置收敛验证记录

本节只记录 2026-07-31 当前工作树和实际运行的新增证据；不追溯性地确认本文其余历史记录。

### 正式配置权威

正式实机入口 `ats_sentry_bringup/launch/bringup.launch.py`、
`real_robot_nav2_free.launch.py` 和 `ats_nav_bringup/launch/rm_navigation_reality_launch.py`
将同一个 `src/ats_sentry_bringup/params/node_params.yaml` 传给 ROGMap、ROGMap
adapter、MINCO、ATS Goal Manager 与全向 SE2 MPC。该 YAML 的正式 MINCO 段令
`goal_topic` 与 `global_plan_topic` 为空，只接收
`/ats_goal_manager/planner_goal`；包内 `*_reality.yaml` 不再是上述正式入口的默认权威。
行为正式配置只保留 `/ats_navigate_to_pose`。

`python3 scripts/validate_navigation_config.py` 已通过；四份受影响 launch 均通过
`py_compile` 和在加载 `install/setup.bash`、`ROS_LOG_DIR=/tmp/ats_show_args_logs` 后的
`ros2 launch ... --show-args`。这些是加载图与静态配置证据，不等价于实机
effective `ros2 param dump`；目标机尚未启动，后者仍为 `未验证`。[Confidence: Medium]

ROGMap 新增 `/rog_map/bounds` `MarkerArray`，RViz profile 明确配置了
`/rog_map/occ`、`/rog_map/inf_occ`、`/rog_map/unk`、`/rog_map/esdf` 与 bounds 的颜色、
尺寸、alpha、frame/QoS 和显示开关。规划仍只消费
`/rog_map/get_ground_projection` 数值服务，不从 `/rog_map/esdf` 反解析距离。

### MuJoCo P3 与安全门禁

所有 P3 用例显式使用
`NAVIGATION_MODE=p3 P3_GOAL_ENTRY=action PLANNING_GRID_OWNER=rog_map`，并在实际 ROS
图中确认无 Nav2 节点和 `/plan`，且 `/rc_esdf/planning_grid`、`/cmd_vel_mpc`、
`/motion_control` 均为唯一所有权。

| ROS_DOMAIN_ID | 用例 | 本轮实际结果 |
| --- | --- | --- |
| 128 | `single` | PASS；终点误差 `0.030675 m`，raw/reference `3/81`，离散 footprint 冲突 `0`。 |
| 129 | `rectangle` | PASS；五段均完成，南北段均观测到横移命令，北段终点误差 `0.002582 m`。 |
| 130 | `red_box` | PASS；终点误差 `0.017337 m`，raw/reference `37/708`，离散 footprint 冲突 `0`。 |
| 131 | `adapter_lease` | PASS；`ready=false -> emergency_stop=true -> cmd_vel_mpc=0 -> motion_control=0`，恢复后旧授权未复活。 |
| 132 | `service_timeout` | PASS；数值 projection 无响应时 adapter not-ready、急停与双级归零，恢复后旧授权未复活。 |
| 133 | `input_stale` | PASS；ROGMap 输入 stale 时急停与双级归零，恢复后旧授权未复活。 |
| 134 | `unknown` | PASS；all-unknown grid 令 adapter not-ready，action 返回码 `4`，双级输出保持零；恢复后 generation 继续递增但无旧授权复活。 |
| 135 | `unreachable` | PASS；空闲但不可达目标 action 返回码 `5`，no-path 急停，双级输出为零；名义前置段终点误差 `0.004196 m`、raw/reference `3/81`。 |
| 136 | Nav2 baseline | PASS；`launch_nav2=true` 下五个 Nav2 lifecycle 节点 active，`NavigateToPose` 成功并发布 local elastic path。此行仅是对照，不能作为 P3 证据。 |

上述 nominal P3 用例均报告 MuJoCo `contact_violation_count=0` 与终局四轮 RPM 低于
`2 rpm`。该计数是仿真诊断，不构成实车物理接触为零的证明；无独立实车 contact
evaluator 时，物理碰撞结论仍为 `未验证`。[Confidence: Medium]

构建/组件证据为：八包最窄构建通过；ATS action GTest
`test_ats_nav_goal_logic`、`test_send_ats_nav_goal_action` 为 `2/2` 通过，覆盖
feedback/result、cancel、preempt、timeout 的判据；`ats_rog_map` CTest 为 `4/4` 通过。
专用 unsafe-trajectory 运行注入与实机 effective 参数导出尚未执行，不能计入已验证安全门禁。
...
6. 实车仅完成 `docs/项目优化文档/nav2移植/p4_real_robot_calibration_preflight.md` 的采集与停止门禁定义，未通电、未运动。
...
2. `decision.topics.cmd_vel`（[ats_sentry_behavior_server.cpp:218](../../../src/ats_sentry_behavior/src/ats_sentry_behavior_server.cpp#L218)）仍暴露行为层直发底盘通路，被 `docs/项目优化文档/nav2移植/p4_real_robot_calibration_preflight.md` 行为决策准入禁止。其余 5.12.1 已记录的缺口（`SyncActionNode` 无 halt、`IsPathGoalReached` 位置判断、硬编码 costmap/odom 输入、无聚焦功能测试）仍全部有效。
...
> 历史证据说明：本节第 1、2 项记录的是 2026-07-28 当时的直连方案，
> 已被 5.16 和第 7 节的最新 frame 契约取代。当前实机默认是
> `/cmd_vel_mpc -> fake-yaw -> chassis transform -> /cmd_vel`，两级开关均为 `True`；
> MuJoCo P3 回归仍直接消费 `/cmd_vel_mpc`，不能用来证明实机兼容层已验收。
...
### 5.16 2026-08-01 P3 MuJoCo ATS action-only 回归入口收敛

状态：`正式 Nav2-free 软件链已实现；Fast DDS 本机 MuJoCo 已完成当前源码下的
Nav2 对照、P3 red_box 名义回归与 unknown/unreachable P2 故障回归；完整 P3 故障、
CycloneDDS 与实车未验收`。本节只记录本轮可复现的源码和运行证据；不覆盖此前历史
记录，也不将单一 DDS 实现或仿真结果写成实车通过。

1. `scripts/test_mujoco_minco_mpc_chain.sh` 已删除 `NAVIGATION_MODE`、
   `P3_GOAL_ENTRY`、Nav2 `NavigateToPose`、`/plan` capture 与 `/goal_pose` fallback。
   它固定传入 `launch_nav2:=false` 和 `launch_trajectory_optimizer:=false`，并且每个
   名义/故障目标均由 `/ats_navigate_to_pose`
   `ats_navigation_interfaces/action/NavigateToPose` 发出。`scripts/test_mujoco_nav_chain.sh`
   仍是唯一显式 Nav2 baseline。
2. P3 脚本固定 `PLANNING_GRID_OWNER=rog_map`，启动后检查 ATS action、禁止 Nav2
   process 与 `/plan`，并验证 adapter 是 `/rc_esdf/planning_grid` 唯一 publisher；
   `/ats_goal_manager/planner_goal`、`/minco/reference_path`、
   `/planner/execution_command`、`/planner/emergency_stop`、`/cmd_vel_mpc` 与
   `/motion_control` 的 owner 也纳入同一回归。
3. 静态、构建与聚焦测试证据已通过：`bash -n scripts/test_mujoco_minco_mpc_chain.sh`、
   `python3 -m py_compile`（五个变更 launch）、
   `python3 scripts/validate_navigation_config.py`、根/导航/行为/MuJoCo 四仓
   `git diff --check`，以及
   `MAKEFLAGS=-j1 colcon build --base-paths src --packages-select
   ats_navigation_interfaces ats_rog_map_interfaces ats_rog_map ats_goal_manager
   ats_sentry_behavior ats_sentry_bringup --parallel-workers 1`（六包成功），随后最新
   源码下 `ats_rog_map`、`ats_nav_bringup`、`ats_sentry_bringup` 与
   `ats_sentry_behavior` 的定向构建也全部通过。`ats_rog_map` 包级测试 `6/6` 通过；
   `ats_goal_manager` 纯生命周期 GTest `2/2` 通过；在工作区 overlay、Fast DDS
   localhost 和 `ROS_LOG_DIR=/tmp` 下，行为侧 `test_send_ats_nav_goal_action` 通过
   `11/11`，`test_send_nav_through_poses_halt` 通过 `4/4`。安装后的
   `ros2 launch ... --show-args` 也成功解析 `bringup.launch.py`、
   `real_robot_nav2_free.launch.py`、`navigation_launch.py`、
   `rm_navigation_reality_launch.py` 与 `rmuc_2026_mujoco.launch.py`。

   完整 `ats_sentry_behavior` 包测试仍被既有 `copyright`、`cpplint`、`xmllint`、
   `clang_format`（69 文件 493 处）与 `black`（未在本轮范围的
   `launch/ats_sentry_behavior_launch.py`）拖红；初次 CTest 的 action/epoch 失败还受
   沙箱只读 `/home/ats/.ros/log` 与 CycloneDDS 无法枚举 interface 影响。未修改这些
   无关文件，功能结果与 lint 结果必须分开阅读。[Confidence: High]
4. R21 速度 frame 契约已完成静态收敛：正式
   `real_robot_nav2_free.launch.py` 默认保持
   `launch_fake_vel_transform:=True` 与
   `launch_chassis_vel_transform:=True`，两级启停不再依赖 `launch_nav2`。正式路由为
   `/cmd_vel_mpc -> fake_vel_transform -> cmd_vel_gimbal_yaw_odom ->
   chassis_vel_transform -> /cmd_vel`，因此 MPC 不再与 chassis transform 竞争
   `/cmd_vel` publisher；固定雷达迁移可显式关闭两级，fake-yaw 关闭时仍由零旋转
   `gimbal_yaw_odom -> gimbal_yaw_fake` TF 保持兼容。安装后的 `--show-args` 与
   validator 均已检查此加载图；`ros2 param dump`、串口输出归零时间和真实云台
   frame 转换仍为`未验证`。[Confidence: Medium]
5. 使用宿主 loopback、`RMW_IMPLEMENTATION=rmw_fastrtps_cpp`、
   `ROS_LOCALHOST_ONLY=1` 的独立 MuJoCo launch 已运行 ATS action-only 名义回归：

   | `ROS_DOMAIN_ID` | profile | 结果 | 终点/轨迹与安全证据 |
   | --- | --- | --- | --- |
   | `104` | `single` | PASS | 终点误差 `0.005965 m`；`3` raw / `81` reference；footprint collisions `0`；telemetry sequence `1126` 后四轮 RPM、命令归零；`contact_violation_count=0`。 |
   | `105` | `rectangle` | PASS | 五段 ATS action 全成功；南北段观测到非零横移 MPC command；最后误差 `0.002472 m`；sequence `1589`；`contact_violation_count=0`。 |
   | `106` | `red_box` | PASS | 预备段误差 `0.007924 m`；红框终点 `(-0.04105,-4.08631)`，误差 `0.006401 m`；`37` raw / `708` reference；footprint collisions `0`；sequence `1758`；`contact_violation_count=0`。 |
   | `112` | `single` 重跑 | PASS | 终点误差 `0.005976 m`；`3` raw / `81` reference；footprint collisions `0`；`/ats_swerve_mpc/reference_horizon` 与 predicted path 均为 `4092` poses；sequence `1037` 后四轮 RPM 与命令归零。 |

   每例都在 action 发出前验证 `launch_nav2=false`、无 Nav2 节点和 `/plan`、四类
   ROGMap 可视化 topic 非空、adapter 为 `/rc_esdf/planning_grid` 唯一 publisher、
   adapter 使用 `/rog_map/get_ground_projection` 而非订阅 `/rog_map/esdf`，并验证
   `/cmd_vel_mpc -> twist_to_motion_ctrl -> /motion_control` 的唯一所有权。`104`--`106`
   的数字来自本轮保存的独立 launch 日志；`112` 是在一次 Fast DDS action response
   超时后用新 domain 的成功重跑。该偶发 response timeout 未复现为源码功能故障，仍是
   DDS 稳定性风险。[Confidence: Medium]
6. 已运行的 P2 故障用例为 `service_timeout`（domain `107`）、`input_stale`
   （domain `109`）与 `adapter_lease`（domain `110`）：均观察到
   `ready=false -> emergency_stop=true -> /cmd_vel_mpc=0 -> /motion_control=0`，恢复后
   generation 分别继续前进而没有新目标驱动旧 reference 复活。`service_timeout` 的
   generation 为 `1073 -> 1126`，`input_stale` 为 `1095 -> 1120`。最后一次脚本与
   速度链修改后使用新 MuJoCo launch 追加执行：domain `183` 的 `unknown` 返回 ATS
   action result `4`，观测到 `ready=false -> emergency_stop=true -> /cmd_vel_mpc=0 ->
   /motion_control=0`，恢复后 generation 前进且旧 reference 未复活；domain `184` 的
   `unreachable` 返回 result `5`，无路时保持 emergency stop，`/cmd_vel_mpc` 与
   `/motion_control` 均为零。`cancel`、`preempt`、`timeout`、server restart 有
   `ats_goal_manager` 聚焦 GTest `11/11` 证据，但尚无本轮同 revision 的跨进程故障运行
   证据。
7. 最后一次源码修改后另以新的 MuJoCo launch 完成两条闭环：domain `181` 的
   `scripts/test_mujoco_nav_chain.sh` Nav2 baseline 使 5 个 lifecycle 节点均进入
   active，`NavigateToPose` 返回 `SUCCEEDED`，local elastic path 非空且未出现空
   `FollowPath`/controller abort；domain `182` 的 P3 `red_box` 预备段误差为
   `0.011246 m`，红框终点误差为 `0.016183 m`，`37` raw / `708` reference，
   footprint collisions `0`、telemetry sequence `1472`、`contact_violation_count=0`，
   终局四轮 RPM 均低于 `2`。上述 MuJoCo 仅使用 Fast DDS localhost，不能外推为实机
   CycloneDDS、无线网络或串口链证据。`contact_violation_count=0` 不是独立物理
   contact evaluator，且 `max_contact_force=0.0`，物理碰撞仍标为`未验证`；脚本也未
   记录端到端 deadline 时延。仍未执行：RViz live 截图、逐节点 `ros2 param dump`、
   P3 action fault、unsafe trajectory 专用运行注入以及所有实机/HIL 验收。
8. 续跑必须在可用宿主 DDS 接口上为每例使用新的 `ROS_DOMAIN_ID` 与新的 MuJoCo
   launch。先执行 P3 action faults 与 unsafe trajectory，再进行 RViz/参数实证；Nav2
   baseline 必须保留为独立脚本，不能作为 P3 ATS action 回归的替代证据。
...
状态：`部分实现`。软件接线与仿真验证见 5.15；实车实测项全部 `未实测`。范围与门禁见 5.14 与 `docs/项目优化文档/nav2移植/p6_real_robot_interface_integration_prompt.md`。

1. 实车 Nav2-free profile：`已实现未运行`。`real_robot_nav2_free.launch.py` 启动 `minco_planner`、`ats_goal_manager`、`ats_swerve_mpc`，固定 `launch_nav2:=false` 与 `use_sim_time:=False`，并默认保留 `launch_fake_vel_transform:=True`、`launch_chassis_vel_transform:=True`。正式 ROGMap、adapter、MINCO、Goal Manager 与 MPC 均从同一份 `node_params.yaml` 取参数。launch 静态展开与配置校验已通过，实车 effective parameter 与运行图 `未验证`。
2. 唯一命令通路：`已实现未运行`。普通实机 profile 为 `/cmd_vel_mpc -> fake-yaw -> cmd_vel_gimbal_yaw_odom -> chassis transform -> /cmd_vel`，`cmd_spin` 关闭；固定雷达 profile 才能显式关闭两级。配置校验已覆盖 Nav2/MPC 与两级开关的 topic 选择；MuJoCo P3 闭环的 `/cmd_vel_mpc` 唯一所有权只证明仿真直接桥接链，不证明实机两级兼容层。
3. 串口层：`部分实现`。授权归零绕过瞬时零保持、`serialPortProtect` 重连与重连期零速度、新 epoch 才恢复授权、符号/单位/轮位口径表均已落地并有单测（`test_cmd_vel_authorization` 14 例通过）；**实测归零时延与实测重连行为 `未实测`**，必须抬轮 HIL 补齐。正式 profile 的 chassis transform 出口限幅已配置为不宽于 MPC 可行域，但符号、限幅与缺反馈归零时延仍需抬轮 HIL 实测。
...
下一对话按"实车不通电检查与抬轮 HIL 优先、P5 并行"的顺序推进。P6 软件侧接线已完成（见 5.15），下一轮的第一件事是**在实车上执行第 1 级不通电检查与第 2 级抬轮 HIL**，按 `docs/项目优化文档/nav2移植/p4_real_robot_calibration_preflight.md` 逐项记录，并补齐本轮标记为 `未实测` 的三类数值：
...
3. 两级速度变换按 frame 契约而非 Nav2 开关分离：普通实机 Nav2-free profile 默认保留 `launch_fake_vel_transform:=True` 与 `launch_chassis_vel_transform:=True`，路径为 `/cmd_vel_mpc -> fake-yaw -> chassis transform -> /cmd_vel`，`cmd_spin` 维持关闭。固定雷达迁移才可显式关闭两级；关闭 fake-yaw 时仍由零旋转兼容静态 TF 提供 `gimbal_yaw_odom -> gimbal_yaw_fake`，且 `base_footprint -> base_link` 不得增加第二个发布者。尚未完成抬轮 HIL 前，兼容层的真实 frame 符号、限幅和失效归零时延均不得写为已验证。
...
ROS_DOMAIN_ID=220 P4_LOCALIZATION_FUSION=true PLANNING_GRID_OWNER=rog_map \
  P2_FAULT_CASE=none P3_FAULT_CASE=none \
  TEST_PROFILE=red_box GOAL_TIMEOUT=180 scripts/test_mujoco_minco_mpc_chain.sh
ROS_DOMAIN_ID=219 P4_LOCALIZATION_FUSION=true PLANNING_GRID_OWNER=rog_map \
  P2_FAULT_CASE=none P3_FAULT_CASE=none \
...
# Nav2 对照仅由 scripts/test_mujoco_nav_chain.sh 执行，不能替代下面的 P3 命令。
...
# P3 action-only：脚本固定 launch_nav2:=false、launch_trajectory_optimizer:=false，
# 且只接受 ats_navigation_interfaces/action/NavigateToPose。
