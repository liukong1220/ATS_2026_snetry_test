...
`AGENTS.md`，并遵循三独立 Git 仓库、最小修改、分仓提交与推送规范。用户可见输出使用中文。

## 0. 当前交接基线

先执行并报告三个仓库的状态、分支、`HEAD` 与 `origin/develop`，建议避免假设工作树干净：
...
以下内容属于用户，除非用户再次明确授权，否则建议避免删除、覆盖、暂存或提交：
...
建议避免替换或删除 Point-LIO、ROGMap、RC-ESDF、JPS、MINCO S3、独立 yaw、footprint gate、
Local Collision Repair、`ats_swerve_mpc`、`standard_robot_pp_ros2`、自瞄相关代码或
`serial/gimbal_joint_state`。四舵轮命令为车体系 `[vx, vy, wz]`，建议避免引入差速、Ackermann、
ICR 或 `vy=0` 约束。传感器配置保持单 LiDAR，参考工程的双雷达方案不在迁移范围内。

CAN、电机、轮速、温度、电压、底盘反馈、硬件 watchdog、HIL 诊断与资源 admission 不属于导航
或仿真验收；默认不重新引入。仿真只验证定位、建图、规划、控制与算法闭环。
...
- `test_rmuc_2025_scene.py` 已通过 `9 passed`，`ats_goal_manager` 与 `minco_planner` 本轮聚焦
  CTest 各 `1/1 passed`；
- `footprint_collisions=0` 不能证明物理未碰撞。没有独立 contact evaluator 时建议写：
  **MuJoCo 物理接触未验证**；
- **已实现未运行（待本轮最终复验）**：实机自主链为
  ...
  `single`/`red_box`/fault matrix 和物理接触均未运行，建议避免写为通过。
...
- **已运行但未通过 P1（2026-08-30，domain `107/113`）**：同一 revision 的两次 headless 运行出现
  结果反转。domain `107` 的 7 级 freshness 全部通过（`gazebo_lidar` p99/max=`0.123249/0.168628 s`），
  但 Point-LIO/ROGMap 出现 Z 轴发散至约 `292.56 m`、`127` 次 out-of-map reset，目标 `ABORTED`；
  domain `113` 目标成功、终点误差 `0.1628 m`，但 `gazebo_lidar` 首违 p99/max=`0.323823/0.349865 s`，
  同时 `livox_input`、`cloud_registered` 和下游阶段也有间隙。当前单次运行不足以判定 freshness owner，
  generic bridge 仍是项目外 owner，P1 保持未通过。artifact 分别为
  `log/gazebo_minco_mpc_chain/20260830_120710_nominal_none_domain107/` 与
  `log/gazebo_minco_mpc_chain/20260830_121706_nominal_none_domain113/`。
- **已修复（分类器阶段表）**：freshness classifier 现在按 `gazebo_transport_lidar -> gazebo_lidar ->
  livox_input -> cloud_registered -> lidar_odometry -> odometry -> localization -> localization_status`
  扫描；旧 domain `228` 的 `lidar_odometry` 首违应视为阶段缺失造成的结构性误标。Transport observer 和
  Direct bridge 在本工作区没有对应运行 artifact，相关判断保留为已实现未运行。
- **已验证（MuJoCo 单点与故障矩阵）**：domain `207` single 终点误差 `0.014508 m`；
...
- **已验证（loopback arbiter 闭环，2026-08-23 domain `222`）**：重装当前 launch 后正式入口启动
  ...
  授权、碰撞或物理动力学。
- 历史 `/cmd_vel_mpc` artifact 只记录迁移前的 freshness/安全收尾；迁移后建议以新 revision、新 ROS domain
...
Claude 的交付范围为实施、测试和结果报告，不包含 Git 写操作。Codex 负责独立 review、显式暂存、分仓提交和普通 push；
文档由 Codex 按已验证证据维护。提交沿用用户既有个人身份，不添加其他作者或 `Co-authored-by`。

P3 尚未验收，建议避免声称 Nav2-free；目标/action 现有功能不等于 P3 已通过。
...
键鼠和 MPC 不直接共同发布同一个底盘话题，因为 `Twist` 没有来源字段且会造成不可判定竞争。
先审查已存在的 arbiter、fake/chassis 变换、launch、`standard_robot_pp_ros2` 串口消费者与
`CmdVelAuthorizationGate`；修复范围聚焦违反以下契约的行为：
...
- Gazebo、MuJoCo、loopback 与实机都不再以 `/cmd_vel_mpc` 作为最终执行入口；MPC 自主输出话题为
  `/cmd_vel/autonomy_raw`，arbiter 的唯一输出为 `/cmd_vel/selected`；
- 键鼠原始 `/cmd_vel` 保持 ROS 默认，用户侧无 remap 要求；
- `/cmd_vel/selected` 仅由 arbiter 发布，串口仅订阅 `/cmd_vel/selected`；
- manual fresh 时优先，manual timeout 后输出零或按明确状态回收，旧手动命令保持失效；
- auto source 依赖 `ExecutionCommand`，manual source 可不依赖该授权；两者在 emergency stop、
  下位机链路失效或各自 timeout 时归零；
- 无链路心跳期间缓存的 manual/auto 命令在首个 DOWN->UP 心跳后保持失效；仅接受 UP 之后到达的
  新命令。该契约由 arbiter 纯逻辑 GTest 锁定；
- fake/chassis 速度变换属于实机默认链，launch 默认仍为
  `launch_fake_vel_transform:=True` 和 `launch_chassis_vel_transform:=True`；
- 回归范围优先覆盖 fresh manual 优先、manual 超时、自动授权、`MODE_STOP`、新 manager incarnation、
  急停/断链/lease 过期两源归零与 selected 唯一 publisher；并在 MuJoCo 或可控 bridge 中实测键鼠
  `/cmd_vel` 到串口出口/`/motion_control` 的闭环。

实现审查重点：

- arbiter 以 `(manager_incarnation, command_sequence)` 识别授权；新 incarnation 先接受新鲜
  `MODE_STOP`、撤销旧租约并作废自动缓存，再接受它的 `MODE_EXECUTE`；旧 incarnation 的 replay 归为拒绝；
- 接受任意 `MODE_STOP` 时保留手动缓存并清空自动缓存，避免随后新 EXECUTE 复活停机前的 Twist；
- 串口 `require_execution_authorization=false` 时，`execution_command_topic` 默认为空，避免串口层
...
- Gazebo 启动 arbiter（无串口场景 `require_serial_link=False`），MPC 发 raw、adapter 订 selected；同步
  ...
  缺失时，adapter 的 `/motion_control` 与 Gazebo chassis 输出同时精确归零；
- loopback 的 `command_topic` 默认及 launch 参数都为 selected，launch 包含
  `cmd_vel_arbiter` 以保持 selected 的唯一 publisher。它不启动 MPC，因此仍只是速度出口回归，不是
  完整导航闭环。

修改前建议按 `AGENTS.md` 给出 DoD、精确文件范围、可执行验证清单、假设和风险转入条件。键鼠运行保持
unknown、地图、footprint、lease、急停和自动导航安全语义不变。

## 3. 第二优先级：RMUC 2025 完整闭环与 P2 故障矩阵

每次运行优先使用全新合法 `ROS_DOMAIN_ID`；残留进程清理范围限定为同 domain，不影响其他用户任务。
所有 colcon 命令建议显式使用 `--base-paths src`，参考目录不是构建输入。
...
`unknown`、`unreachable`。每项建议观察并保存：

```text
ready=false -> emergency_stop=true -> 最终速度=0 -> /motion_control=0
```

恢复后 source/adapter generation 建议递增；未发送新目标时，迟到 response、旧 snapshot 或急停前
reference 保持失效。成功路线建议记录最终坐标、误差、raw/reference 点数、离散 footprint 冲突采样数、
失败/恢复次数；物理接触单独标注为未验证，除非加入独立 contact evaluator。

## 4. 第三优先级：以 `参考/navi_minco_bit` 做证据型审阅

参考项目路径为 `~/ATS_2026_snetry_test/参考/navi_minco_bit`，仅作为算法和实现证据来源，
不进入 colcon 构建；整包、双雷达、Nav2 依赖和通信协议均不在迁移范围内。优先审阅：
...
围绕四条线逐项建立表格，每一项建议包含“参考源码证据 -> ATS 当前实现 -> 实际缺口 -> 最小改动 ->
可运行测试”，建议避免把参考工程中存在的代码当作 ATS 性能结果：
...
最后分别检查根仓、导航仓、MuJoCo 仓的 `git diff --check`。Claude 向 Codex 交付精确文件、diff 要点和原始测试输出，
Git 写入由 Codex 负责，并按接口/算法/仿真/文档拆分中文提交后普通 push。
Codex 提交前建议核对 `git diff --cached --stat` 与 `git diff --cached --check`，仅显式暂存本轮文件，不使用
`git add .`、`git add -A`、force push，且不添加其他作者。最终报告建议区分“已验证”“已实现未运行”“推断”“未实现”，
