...
- 关于路径规划：
    ...
    使用 NAV2 默认的 Global Planner（SmacPlannerHybrid, A*）作为全局路径规划器，nav2_mppi_controller（MPPI, 模型预测路径积分控制器）作为局部路径跟踪器。MPPI 通过采样和滚动优化实现预测性控制，在高速和急转弯场景下相比 PID 纯追踪控制器具有更好的跟踪精度和更少的超调。
