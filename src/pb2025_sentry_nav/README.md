...
- [pb2025_nav_bringup/launch/rm_navigation_reality_launch.py](./pb2025_nav_bringup/launch/rm_navigation_reality_launch.py)

说明：

- 当前整车维护优先使用工作区总入口 `pb2025_sentry_bringup/bringup.launch.py`
...
├── pb2025_nav_bringup/            # 导航 launch、RViz、simulation/reality 参数
...
- [../pb2025_sentry_bringup/params/node_params.yaml](../pb2025_sentry_bringup/params/node_params.yaml)

这份文件比 `pb2025_nav_bringup/config/reality/nav2_params.yaml` 更重要，因为它是当前工作区总入口真实加载的参数。

### reality 默认参数

- [pb2025_nav_bringup/config/reality/nav2_params.yaml](./pb2025_nav_bringup/config/reality/nav2_params.yaml)

当前主要用于：

1. 导航子系统单独启动
2. 与 `node_params.yaml` 对照
3. 保留 `pb2025_nav_bringup` 包内默认配置
...
3. 如果问题是“姿态/视觉/目标点异常”，不要先改本包，优先看 `pb2025_sentry_behavior`
4. 如果问题是“最终底盘速度和 Nav2 输出不一致”，同时看 `fake_vel_transform` 与 `standard_robot_pp_ros2`
5. 若只改了 `pb2025_nav_bringup/config/reality/nav2_params.yaml` 却发现总入口没变化，先确认当前是不是从 `node_params.yaml` 启动的
