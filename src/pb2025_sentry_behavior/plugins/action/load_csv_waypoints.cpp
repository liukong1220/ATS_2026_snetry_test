...
std::string resolveCsvPath(const std::string & filepath, const rclcpp::Logger & logger)
{
  ...
  try {
    const auto share_dir = ament_index_cpp::get_package_share_directory("pb2025_sentry_behavior");
    return (std::filesystem::path(share_dir) / filepath).string();
  } catch (const std::exception & ex) {
    RCLCPP_WARN(logger, "Failed to resolve pb2025_sentry_behavior share directory: %s", ex.what());
    return filepath;
  }
}
