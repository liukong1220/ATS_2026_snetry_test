...
def generate_launch_description():
    ...
    nav_bringup_dir = get_package_share_directory("ats_nav_bringup")
    ...
    declare_nav_world = DeclareLaunchArgument(
        "nav_world",
        default_value="rmuc_2025",
        description="Navigation map/pcd asset basename under ats_sentry_bringup/{map,pcd}",
    )
    ...
    declare_robot_xmacro_file = DeclareLaunchArgument(
        "robot_xmacro_file",
        default_value=os.path.join(
            get_package_share_directory("ats_robot_description"),
            "resource",
            "xmacro",
    ...
    behavior_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                behavior_bringup_dir, "launch", "ats_sentry_behavior_launch.py"
            )
        ),
