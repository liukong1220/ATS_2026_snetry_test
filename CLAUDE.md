...
- 所有任务一律在当前分支（`develop`）上提交，禁止为任何任务新开分支。
- 不得执行 `git checkout -b`、`git switch -c`、`git branch <new>` 等创建分支的操作；后续所有开发与提交均在当前分支进行。
- 若某任务确实需要独立分支，必须先向用户说明并获得显式许可，默认按“不新开分支”执行。
